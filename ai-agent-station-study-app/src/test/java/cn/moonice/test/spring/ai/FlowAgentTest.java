package cn.moonice.test.spring.ai;

import cn.moonice.test.spring.ai.advisors.RagAnswerAdvisor;
...
public class FlowAgentTest {
    ...
    private OpenAiApi sharedOpenAiApi;
    private McpSyncClient baiduSearchMcpClient;
    private McpSyncClient csdnMcpClient;
    private McpSyncClient weixinMcpClient;
    ...
    @BeforeEach
    public void init() {
        sharedOpenAiApi = OpenAiApi.builder()
        ...
        baiduSearchMcpClient = sseMcpClient_BaiduSearch();
        csdnMcpClient = sseMcpClient_csdn();
        weixinMcpClient = sseMcpClient02_weixin();

        chatModel = OpenAiChatModel.builder()
                .openAiApi(sharedOpenAiApi)
                .defaultOptions(OpenAiChatOptions.builder()
                        .model("gpt-4.1-mini")
                        .maxTokens(5000)
                        .toolCallbacks(new SyncMcpToolCallbackProvider(baiduSearchMcpClient, csdnMcpClient, weixinMcpClient).getToolCallbacks())
                        .build())
                .build();
        ...
        executorChatClient = ChatClient.builder(chatModel)
                .defaultSystem(executorSystemPrompt)
                .defaultAdvisors(
                        PromptChatMemoryAdvisor.builder(
                                MessageWindowChatMemory.builder()
                                        .maxMessages(20)
    ...
    private void executeStepsInOrder(Map<String, String> stepsMap, Map<String, Object> executionContext) {
        ...
        executionContext.put("calledTools", calledTools);
    ...
    private void executeStep(Integer stepNumber, String stepKey, String stepContent, Map<String, Object> executionContext) {
        ...
        try {
            ...
            List<String> allowedTools = extractToolNames(stepContent);
            @SuppressWarnings("unchecked")
            List<String> calledTools = (List<String>) executionContext.getOrDefault("calledTools", new ArrayList<String>());

            List<String> toolsToRegister = new ArrayList<>();
            for (String tool : allowedTools) {
                if (!calledTools.contains(tool)) {
                    toolsToRegister.add(tool);
                }
            }

            log.info("步骤 {} 允许的工具: {}, 已调用过的工具: {}, 本次注册的工具: {}",
                    stepNumber, allowedTools, calledTools, toolsToRegister);

            ChatClient stepChatClient = buildStepChatClient(toolsToRegister);

            String executionResult = executeWithRetry(() -> {
                return stepChatClient.prompt()
                        .user(buildStepExecutionPrompt(stepContent, executionContext, toolsToRegister, calledTools))
                        .call()
                        .content();
            }, "步骤" + stepNumber + "执行", 2);
    ...
    private String buildStepExecutionPrompt(String stepContent, Map<String, Object> executionContext,
                                             List<String> toolsToRegister, List<String> calledTools) {
        StringBuilder prompt = new StringBuilder();
        prompt.append("你是一个智能执行助手，需要执行以下步骤:\n\n");
        prompt.append("**步骤内容:**\n");
        prompt.append(escapeSt4(stepContent)).append("\n\n");

        prompt.append("**用户原始请求:**\n");
        prompt.append(escapeSt4(String.valueOf(executionContext.get("userRequest")))).append("\n\n");

        if (!toolsToRegister.isEmpty()) {
            prompt.append("**本步骤可用的工具:**\n");
            for (String tool : toolsToRegister) {
                prompt.append("- ").append(tool).append("\n");
            }
            prompt.append("\n");
        } else {
            prompt.append("**本步骤没有可用的工具**（所需工具已在之前步骤中调用过，或本步骤不需要工具）\n");
            prompt.append("请根据已有结果直接生成本步骤的执行报告，不要尝试调用任何工具。\n\n");
        }

        if (!calledTools.isEmpty()) {
            prompt.append("**已在之前步骤中调用过的工具（不可再次调用）:**\n");
        ...
        prompt.append("2. 只使用本步骤可用的工具，不要调用其他工具\n");
    ...
    private String getMcpToolsCapabilities(String userRequest) {
        String escapedRequest = escapeSt4(userRequest);
    ...
    private String getActualMcpToolsInfo() {
        ...
        try {
            ...
            toolsInfo.append("- 工具名称: JavaSDKMCPClient_baiduSearch\n");
            ...
            toolsInfo.append("- 工具名称: JavaSDKMCPClient_saveArticle\n");
            toolsInfo.append("- 参数: title (String) - 文章标题, content (String) - 文章内容, tags (String) - 标签\n");
            toolsInfo.append("- 适用场景: 技术文章发布、知识分享、内容创作\n\n");

            toolsInfo.append("## 3. 微信通知工具 (Weixin)\n");
            toolsInfo.append("- 核心功能: 通过微信发送消息通知\n");
            toolsInfo.append("- 工具名称: JavaSDKMCPClient_weixinNotice\n");
            toolsInfo.append("- 参数: platform, title, description, url 等\n");
            toolsInfo.append("- 适用场景: 任务完成通知、状态更新提醒\n\n");

            toolsInfo.append("## 工具组合使用模式\n");
            toolsInfo.append("1. 信息收集阶段: 使用baiduSearch检索相关技术资料\n");
            toolsInfo.append("2. 内容发布阶段: 使用saveArticle发布文章到CSDN\n");
            toolsInfo.append("3. 通知推送阶段: 使用weixinNotice发送完成通知（与发布在同一步骤完成）\n\n");

        } catch (Exception e) {
            log.warn("获取MCP工具信息时发生错误: {}", e.getMessage());
            toolsInfo.append("注意: 部分工具信息获取失败，请检查MCP服务连接状态\n");
        }
    ...
    private String buildStructuredPlanningPrompt(String userRequest, String mcpToolsAnalysis) {
        ...
        prompt.append(escapeSt4(userRequest));
        prompt.append("\n\n");

        prompt.append("**重要提醒：** 在生成执行计划时，必须完整保留和传递用户请求中的所有详细信息。\n\n");

        prompt.append("## MCP工具能力分析结果\n");
        prompt.append(escapeSt4(mcpToolsAnalysis));
        prompt.append("\n\n");
        ...
        prompt.append(escapeSt4(actualToolsInfo)).append("\n");
        ...
        prompt.append("### 通知合并原则（重要）\n");
        prompt.append("- 通知类工具（如weixinNotice）不得作为独立步骤\n");
        prompt.append("- 发布+通知必须在同一步骤中完成\n");
        prompt.append("- 例如：发布文章到CSDN后，立即在同一步骤中发送微信通知\n");
        prompt.append("- 不要将'发送通知'单独拆分为一个步骤\n\n");
        ...
        prompt.append("- **发布+通知步骤**: 发布内容后立即发送通知，不要拆分\n\n");
    ...
    public McpSyncClient sseMcpClient_csdn() {
        ...
        HttpClientSseClientTransport sseClientTransport = HttpClientSseClientTransport.builder("http://106.53.86.136:8101").build();
    ...
    public McpSyncClient sseMcpClient02_weixin() {
        ...
        HttpClientSseClientTransport sseClientTransport = HttpClientSseClientTransport.builder("http://106.53.86.136:8102").build();
    ...
    private String escapeSt4(String text) {
        if (text == null) return "";
        return text.replace("\\", "\\\\").replace("{", "\\{").replace("}", "\\}");
    }

    private List<String> extractToolNames(String stepContent) {
        List<String> toolNames = new ArrayList<>();
        Pattern toolPattern = Pattern.compile("(JavaSDKMCPClient_\\w+|functions\\.JavaSDKMCPClient_\\w+)");
        Matcher matcher = toolPattern.matcher(stepContent);
        while (matcher.find()) {
    ...
    private ChatClient buildStepChatClient(List<String> allowedTools) {
        List<McpSyncClient> mcpClients = new ArrayList<>();

        for (String toolName : allowedTools) {
            if (toolName.contains("saveArticle") && !mcpClients.contains(csdnMcpClient)) {
                mcpClients.add(csdnMcpClient);
            } else if (toolName.contains("weixinNotice") && !mcpClients.contains(weixinMcpClient)) {
                mcpClients.add(weixinMcpClient);
            } else if (toolName.contains("baiduSearch") && !mcpClients.contains(baiduSearchMcpClient)) {
                mcpClients.add(baiduSearchMcpClient);
            }
        }
        ...
        if (!mcpClients.isEmpty()) {
            ToolCallback[] allToolCallbacks = new SyncMcpToolCallbackProvider(
                    mcpClients.toArray(new McpSyncClient[0])).getToolCallbacks();

            List<String> allCallbackNames = new ArrayList<>();
            for (ToolCallback callback : allToolCallbacks) {
                allCallbackNames.add(callback.getToolDefinition().name());
            }
            log.info("注册的全部工具名称: {}", allCallbackNames);
            ...
            for (ToolCallback callback : allToolCallbacks) {
                ...
                for (String allowedTool : allowedTools) {
                    String shortName = allowedTool.replace("JavaSDKMCPClient_", "");
                    if (callbackName.equals(allowedTool) || callbackName.equals(shortName) || callbackName.contains(shortName)) {
                        filteredCallbacks.add(callback);
                        log.info("工具匹配成功: callbackName={}, allowedTool={}, shortName={}", callbackName, allowedTool, shortName);
                        break;
                    }
                }
            }

            log.info("工具过滤 - 全部工具数: {}, 过滤后工具数: {}, 允许的工具: {}",
                    allToolCallbacks.length, filteredCallbacks.size(), allowedTools);

            if (!filteredCallbacks.isEmpty()) {
                optionsBuilder.toolCallbacks(filteredCallbacks.toArray(new ToolCallback[0]));
            }
        }

        OpenAiChatModel stepChatModel = OpenAiChatModel.builder()
                .openAiApi(sharedOpenAiApi)
                .defaultOptions(optionsBuilder.build())
                .build();

        return ChatClient.builder(stepChatModel)
                .defaultSystem(executorSystemPrompt)
                .defaultAdvisors(
                        PromptChatMemoryAdvisor.builder(
                                MessageWindowChatMemory.builder()
